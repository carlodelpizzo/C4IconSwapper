...
from tkinter import Button, Canvas, Checkbutton, Entry, Frame, Menu, OptionMenu, Text, Toplevel
...
sm_icon_size, lg_icon_size = (60, 60), (128, 128)
...
class C4IconSwapper:
    def __init__(self):
        ...
        self.name = StringVar()
        self.name_new_prev = 'New Driver'
        self.name_new = StringVar(value=self.name_new_prev)
        ...
        self.version_new = StringVar(value=self.version_new_prev)
        ...
        with Image.open(assets_path / 'blank_img.png') as img:
            self.img_blank = ImageTk.PhotoImage(img.resize(lg_icon_size))
            self.img_bank_blank = ImageTk.PhotoImage(img.resize(sm_icon_size))
    ...
    def blink_driver_name_entry(self, *_):
        if not self.counter:
            return
        self.counter -= 1
        if self.export_panel.file_name_entry['background'] != light_entry_bg:
            self.export_panel.file_name_entry['background'] = light_entry_bg
        else:
    ...
    def load_project(self, *_, path_str='', scheduled=False, merge_project=None):
        ...
        with Image.open(assets_path / 'loading_img.png') as img:
            ...
            self.c4z_panel.c4_icon_label.config(image=icon)
            self.root.update_idletasks()

        def restore_label_img(update_idletasks=True):
            self.c4z_panel.c4_icon_label.config(image=curr_label_img)
            if update_idletasks:
                self.root.update_idletasks()
        ...
        if c4is.driver_selected or not merge_project:
            ...
            self.c4z_panel.c4_icon_label.config(image=self.img_blank)
        ...
        if not merge_project:
            ...
            for img_bank_label in self.replacement_panel.img_bank_labels:
                img_bank_label.config(image=self.img_bank_blank)
        ...
        if c4is.replacement:
            if merge_project and self.replacement_panel.replacement_icon:
                ...
                self.replacement_panel.replacement_label.config(image=self.replacement_panel.replacement_icon.icon_lg)
    ...
    def do_recovery(self, recover_path: Path):
        ...
        if recovery_state:
            ...
            for conn_dict in recovery_state['new_conns']:
                ...
                conn.enabled = True
                conn.tag.hide = False
                conn.prev_name = conn_dict['name']
                conn.name.set(conn.prev_name)
                conn.prev_type = conn_dict['type']
                conn.type.set(conn.prev_type)
            ...
            # Driver Info
            self.name_new_prev = recovery_state['name']
            self.name_new.set(self.name_new_prev)
            self.manufac_new_prev = recovery_state['manufac']
            self.manufac_new.set(self.manufac_new_prev)
            self.creator_new_prev = recovery_state['creator']
            self.creator_new.set(self.creator_new_prev)
            self.version_new_prev = recovery_state['version']
            self.version_new.set(self.version_new_prev)

            # Export Panel
            self.export_panel.file_name_prev = recovery_state['file_name']
            self.export_panel.file_name.set(self.export_panel.file_name_prev)
            self.export_panel.use_orig_xml_prev = recovery_state['use_orig_xml']
            self.export_panel.use_orig_xml.set(self.export_panel.use_orig_xml_prev)
            self.export_panel.include_backups_prev = recovery_state['include_backups']
            self.export_panel.include_backups.set(self.export_panel.include_backups_prev)
            self.export_panel.inc_driver_version_prev = recovery_state['inc_driver_version']
    ...
    def undo(self, *_, redo=False):
        ...
        match undo_dict:
            ...
            case {'conn_change': 'user'}:
                if undo_dict.get('add_conn'):
                    conn_obj = self.connections.pop(-2)
                    self.driver_xml.get_tag('connections').remove_element(conn_obj.tag)
                    if (in_dict := self.taken_xml_ids.get(conn_obj.id)) and conn_obj is in_dict:
                        self.taken_xml_ids.pop(conn_obj.id)
                    if self.connections_win:
                        self.connections_win.suppress_trace = True
                        conn_entry = self.connections_win.connection_entries.pop(-2)
                        conn_entry.destroy()
                        self.connections_win.connection_entries[-1].y -= self.connections_win.widget_y_offset
                        self.connections_win.connection_entries[-1].refresh()
                        self.connections_win.update_scroll_frame()
                        self.connections_win.suppress_trace = False
                elif (i := undo_dict.get('x_conn')) is not None:
                    new_conn = Connection(self)
                    self.connections.insert(i, new_conn)
                    self.driver_xml.get_tag('connections').add_element(new_conn.tag)
                    new_conn.enabled = True
                    new_conn.tag.hide = False
                    new_conn.update_id()
                    new_conn.prev_name = undo_dict['name']
                    new_conn.name.set(new_conn.prev_name)
                    new_conn.prev_type = undo_dict['type']
                    new_conn.type.set(new_conn.prev_type)
                    if self.connections_win:
                        self.connections_win.suppress_trace = True
            ...
            case {'dinfo_change': 'name'}:
                self.name_new_prev = undo_dict['name']
                self.name_new.set(self.name_new_prev)
        ...
        # Open any windows where changes have been made
        match undo_dict:
            case {'conn_change': _}:
...
class SettingsWin:
    def __init__(self, main: C4IconSwapper):
        ...
        self.window.bind('<Control-z>', main.undo)
...
class Icon:
    def __init__(self, path: Path, img=None):
        ...
        def create_tk_icons(pil_img):
            self.icon_lg = ImageTk.PhotoImage(pil_img.resize(lg_icon_size, Resampling.BICUBIC))
            self.icon_sm = ImageTk.PhotoImage(pil_img.resize(sm_icon_size, Resampling.BICUBIC))
        if img:
            create_tk_icons(img)
        else:
...
class C4Icon(Icon):
    def __init__(self, main, icons: list[C4SubIcon] | set[C4SubIcon], parent_tag: XMLTag = None,
        ...
        if self.bak:
            ...
            with Image.open(icon.bak_path) as img:
                self.bak_tk_icon = ImageTk.PhotoImage(img.resize(lg_icon_size, Resampling.BICUBIC))
    ...
    def refresh_tk_img(self, bak=False):
        ...
        with Image.open(self.path) as img:
            self.icon_lg = ImageTk.PhotoImage(img.resize(lg_icon_size, Resampling.BICUBIC))
...
class SubIconWin:
    ...
    def update_icon(self):
        ...
        with Image.open(curr_sub_icon.path) as img:
            self.sub_icon_label.config(image=ImageTk.PhotoImage(img.resize(lg_icon_size, Resampling.BICUBIC)))
...
class DriverInfoWin:
    ...
    def update_undo_history(self, focus_in: bool, entry: str):
        match entry:
            case 'name':
                if focus_in:
                    self.main.name_new_prev = self.main.name_new.get()
                    return
                elif self.main.name_new_prev != self.main.name_new.get():
    ...
    def update_entries(self):
        ...
        self.name_new_entry.config(
            textvariable=(main.name if use_orig else main.name_new), state=entry_state)
    ...
    def close(self, *_):
        self.main.name_new.trace_remove('write', self.trace_n)
        ...
        changes = {
            'name': self.main.name_new_prev != self.main.name_new.get(),
            'manufac': self.main.manufac_new_prev != self.main.manufac_new.get(),
            'creator': self.main.creator_new_prev != self.main.creator_new.get(),
            'version': self.main.version_new_prev != self.main.version_new.get()
        }

        if change_type := next((k for k, v in changes.items() if v), None):
            self.main.update_undo_history({'dinfo_change': change_type})
            match change_type:
                case 'name':
                    self.main.name_new_prev = self.main.name_new.get()
                case 'manufac':
...
# TODO: Check if conn name is saved on exit if still focused in entry
class ConnectionsWin:
    ...
    def on_resize(self, *_, called_after=False):
        if self.suppress_trace or (new_size := (self.window.winfo_width(), self.window.winfo_height())) == self.size:
            return
        if not called_after:
...
class ConnectionEntry:
    def __init__(self, parent: ConnectionsWin, conn_obj: Connection, x_pos: int, y_pos: int):
        ...
        self.name_entry['state'] = 'readonly' if self.conn_obj.original else button_state
    ...
    def action(self, command=''):
        match (command or self.action_button['text']):
            case 'Add':
                self.conn_obj.enabled = True
                self.conn_obj.tag.hide = False
                self.conn_obj.update_id()
    ...
    def type_update(self, *_):
        if self.parent.suppress_trace:
            return
        self.conn_obj.update_id()
        self.update_undo_history(False, entry=False)
        self.main.ask_to_save = True
...
class RecoveryWin:
    def __init__(self, main: C4IconSwapper, *_):
        ...
        self.window.focus()
        self.window.transient(main.root)
        self.window.title('Project Recovery')
        self.window.geometry(f'355x287+{main.root.winfo_rootx()}+{main.root.winfo_rooty()}')
        self.window.resizable(False, False)
        self.window.protocol('WM_DELETE_WINDOW', self.close_dialog)
        self.window.bind('<Control-w>', self.close_dialog)

        self.recoverable_projects = [rec_obj for path in recovery_dir.iterdir() if (rec_obj := RecoveryObject(path))]

        title_label = Label(self.window, text='Select Projects to Recover', font=(label_font, 14))
        ...
        recover_button = Button(self.window, text='Recover', width=10, command=self.do_recovery)
        recover_button.place(relx=0.5, x=-4, rely=1, y=-3, anchor='se')

        delete_button = Button(self.window, text='Delete' if num_of_rec_folders == 1 else 'Delete All',
...
class C4zPanel:
    def __init__(self, main: C4IconSwapper):
        ...
        # Buttons
        self.open_button = Button(main.root, text='Open', width=10, command=self.load_c4z, takefocus=0)
        self.open_button.place(x=187 + self.x, y=30 + self.y, anchor='w')
        ...
        self.prev_button = Button(main.root, text='Prev', command=lambda: self.inc_icon(inc=-1), width=5, takefocus=0)
        self.prev_button.place(x=180 + self.x, y=146 + self.y)
        self.prev_button['state'] = DISABLED

        self.next_button = Button(main.root, text='Next', command=self.inc_icon, width=5, takefocus=0)
        self.next_button.place(x=230 + self.x, y=146 + self.y)
        self.next_button['state'] = DISABLED
    ...
    def load_gen_driver(self, multi=False):
        ...
        def gen_driver_unpack():
            ...
            with Image.open(assets_path / 'loading_img.png') as loading_img:
                self.c4_icon_label.config(image=ImageTk.PhotoImage(loading_img))
    ...
    def _load_c4z(self, file_path=None, generic=False, c4is=None):
        ...
        use_default_use_original_xml = self.icons and not main.driver_xml
        ...
        def get_icon_groups_from_xml() -> dict[tuple[str, XMLTag], set[Path]]:
            ...
            for tag in main.driver_xml.get_tags('proxy'):
                if proxybindingid := tag.attributes.get('proxybindingid'):
                    proxy_binding_id_dict[proxybindingid] = tag.attributes.get('name')
        ...
        if abort:
            ...
            if abort == 'xml':  # noinspection PyUnboundLocalVariable
                ...
                self.icons = []
        elif use_default_use_original_xml:
        ...
        self.prev_button['state'] = prev_next_button_state
        self.next_button['state'] = prev_next_button_state
    ...
    def update_icon(self):
        ...
        self.c4_icon_label.config(image=icons[self.current_icon].tk_img)
...
class ReplacementPanel:
    def __init__(self, main: C4IconSwapper):
        ...
        for i in range(main.img_bank_size):
            ...
            bank_label.place(x=31 + self.x + label_spacing * i, y=176 + self.y, anchor='nw')
        ...
        # Buttons
        self.open_button = Button(main.root, text='Open', width=10, command=self.process_image, takefocus=0)
        self.open_button.place(x=187 + self.x, y=30 + self.y, anchor='w')
        ...
        self.prev_button = Button(
            main.root, text='Prev', command=lambda: self.inc_img_bank(inc=-1), width=5, takefocus=0)
        self.prev_button.place(x=180 + self.x, y=146 + self.y)
        self.prev_button['state'] = DISABLED

        self.next_button = Button(main.root, text='Next', command=self.inc_img_bank, width=5, takefocus=0)
        self.next_button.place(x=230 + self.x, y=146 + self.y)
        self.next_button['state'] = DISABLED
                        ...
                        if bank_index is not None:
                            ...
                            if existing_replacement and self.replacement_icon.path == cmp_path:
                                if drop_on_blank:
                                    ...
                                    self.replacement_icon = None
                                    self.replacement_label.config(image=main.img_blank)
                                    bank_index += 1
                                    self.replace_button['state'] = DISABLED
                                    self.replace_all_button['state'] = DISABLED
                                    self.file_entry_field['state'] = DISABLED
                                    ...
                                    self.replacement_label.config(image=self.replacement_icon.icon_lg)
                                self.img_bank_labels[bank_index].config(image=self.img_bank[bank_index].icon_sm)
                                return
                            ...
                            self.img_bank_labels[existing_index].config(image=swap_icon.icon_sm)
                            self.img_bank_labels[bank_index].config(image=existing_icon.icon_sm)
                            return
                            ...
                            if existing_replacement:
                                ...
                                self.img_bank[bank_index] = rp_icon
                                self.img_bank_labels[bank_index].config(image=rp_icon.icon_sm)
                                ...
                                if bank_index <= (bank_len := len(self.img_bank)):
                                    for i, tk_label in enumerate(self.img_bank_labels[bank_index:bank_len]):
                                        tk_label.config(image=self.img_bank[i + bank_index].icon_sm)
                                    last_index = bank_len if bank_len < main.img_bank_size else -1
                                    self.img_bank_labels[last_index].config(image=main.img_bank_blank)
                                self.update_buttons()
                            self.replacement_label.config(image=self.replacement_icon.icon_lg)
                            if c4z_drop and main.c4z_panel.icons:
                                self.replace_icon(update_ask_to_save=False)
    ...
    def process_image(self, file_path: Path | str | list | tuple = None,
        ...
        if self.open_button.config('relief')[-1] == 'sunken':
            return
        if new_thread:
        ...
        if not self.threading_event.is_set():
            self.open_button.config(relief='sunken')
    ...
    def on_tk_label(self, tk_label):
        in_x_range = tk_label.winfo_x() <= self.mouse_pos[0] <= tk_label.winfo_x() + tk_label.winfo_width()
        in_y_range = tk_label.winfo_y() <= self.mouse_pos[1] <= tk_label.winfo_y() + tk_label.winfo_height()
    ...
    def on_click(self, event, bank_num=-1):
        ...
        scale_factor = sm_icon_size[0] / lg_icon_size[0] if bank_num < 0 else 1
        self.drag_offset = (event.x * scale_factor, event.y * scale_factor)

        drag_image = self.replacement_icon.icon_sm if bank_num < 0 else self.img_bank[bank_num].icon_sm
        self.drag_label = Label(self.main.root, image=drag_image)
        ...
        if self.replace_button['state'] != DISABLED and self.on_tk_label(self.main.c4z_panel.c4_icon_label):
            ...
            if drop_index > len(self.img_bank) - 1:
                if self.selected_img < 0 and len(self.img_bank) < self.main.img_bank_size:
                    ...
                    self.img_bank_labels[len(self.img_bank)].config(image=self.replacement_icon.icon_sm)
                    self.img_bank.append(self.replacement_icon)
                    self.replacement_icon = None
                    end_drag()
                    return
            ...
            if self.selected_img < 0:
                ...
                self.img_bank_labels[self.selected_img].config(image=self.img_bank[self.selected_img].icon_sm)
            self.img_bank[drop_index] = selected_icon
            dropped_label.config(image=self.img_bank[drop_index].icon_sm)
    ...
    def on_drag(self, event):
        ...
        elif self.dragging and self.drag_label:
    ...
    def add_to_img_bank(self, img: Icon, bank_index=None):
        ...
        if bank_index is not None:
            ...
            if bank_len < self.main.img_bank_size:
                self.img_bank_labels[bank_len].config(image=self.img_bank[bank_len].icon_sm)
    ...
    def refresh_img_bank(self):
        bank_len, img_blank = len(self.img_bank), self.main.img_bank_blank
        for i, bank_label in enumerate(self.img_bank_labels):
            bank_label.config(image=self.img_bank[i].icon_sm if i < bank_len else img_blank)

    def inc_img_bank(self, inc=1):
        if not self.threading_event.is_set():
            return
        if not inc or len(self.img_bank) <= self.main.img_bank_size:
            return
        self.img_bank = self.img_bank[-inc:] + self.img_bank[:-inc]
    ...
    def delete_image(self, img_index: int, update_undo_history=True):
        ...
        if bank_len < self.main.img_bank_size:
            ...
            self.img_bank_labels[last_index].config(image=self.main.img_bank_blank)
        for i, tk_label in enumerate(self.img_bank_labels[img_index:bank_len]):
            tk_label.config(image=self.img_bank[i + img_index].icon_sm)
    ...
    def update_buttons(self):
        ...
        self.next_button['state'] = next_button_state
        self.prev_button['state'] = next_button_state
...
class ExportPanel:
    def __init__(self, main: C4IconSwapper):
        ...
        self.driver_name_label = Label(main.root, text='File Name:')
